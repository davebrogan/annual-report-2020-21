...
window.addEventListener('load', function(e) {
    ...
    if (vpAspect()) {
        bannerWrap.style.backgroundPositionY = -bgOffsetY + 'px';
        ...
        window.addEventListener('scroll', function(e) {
            ...
            if (knockoutPosTop <= bgOffsetY && knockoutPosTop > 0) {
                ...
                bannerWrap.style.backgroundPositionY = -(verticalBgSlider) + (bgOffsetMakeUp/verticalBgSlider) + 'px';
            } else if (window.pageYOffset > vh || knockoutPosTop === 0) {
                //this freezes the bg image movement if the knockout has reached the top
                bannerWrap.style.backgroundPositionY = bgOffsetMakeUp + 'px';
            }
        })
    } else {
...
window.addEventListener('scroll', function() {
    let firstTitlePosTop = firstTitle.getBoundingClientRect().top; //distance knockout image from window top
    //show/hide first-title
    if (firstTitlePosTop <= 0) {
        //firstTitle.classList.add('animate-up');
        firstTitle.classList.add('freeze-knockout');
    } else if (firstTitlePosTop > 0 && firstTitle.classList.contains('freeze-knockout')) {
        firstTitle.classList.remove('freeze-knockout');
    } else if (firstTitlePosTop >= origPos) {
